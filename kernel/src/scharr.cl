...
//TODO: check if image reads can be made more simultaneously to boost performance
...
// [1] Out	dst_image: 4 channel image of x and y gradient (SFLOAT), angle (SNORM),
...
__kernel void scharr(read_only image2d_t src_image, write_only image2d_t dst_image)
{
	...
	diag = read_imagef(src_image, sampler, coords - 1).x;
	diag -= read_imagef(src_image, sampler, coords + 1).x;
	...
	diag = read_imagef(src_image, sampler, coords - (int2)(1,-1)).x;
	diag -= read_imagef(src_image, sampler, coords + (int2)(1,-1)).x;
