	...
	write_only image2d_t ui4_arc_data)
{
	...
	while((cont_data & 0xC8) == 0x08)	// check start flag and forced end flag not set, and continuation flag set
	{
		...
		if(max_accel - min_accel > ACCEL_THRESH)
		{
			...
			write_data_accum_alt(path_length, ui4_arc_data, base_coords.i, prev_coords.i, offset_mid);
			// reset path_accum
	...
	write_data_accum_alt(path_length, ui4_arc_data, base_coords.i, coords.i, offset_mid);
}